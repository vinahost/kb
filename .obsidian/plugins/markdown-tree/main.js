...
__export(main_exports, {
  default: () => MarkdownTree
});
module.exports = __toCommonJS(main_exports);
var import_obsidian = require("obsidian");

// api/utilities/generate-tree.ts
var import_lodash = __toESM(require_lodash());

// api/consts/line-strings.ts
var LINE_STRINGS = {
  ascii: {
    CHILD: "|-- ",
    LAST_CHILD: "`-- ",
...
var generateTree = (structure, options) => (0, import_lodash.flattenDeep)([
  getAsciiLine(structure, (0, import_lodash.defaultsDeep)({}, options, defaultOptions)),
  structure.children.map((c) => generateTree(c, options))
]).filter((line) => line != null).join("\n");
var getAsciiLine = (structure, options) => {
...
var getName = (structure, options) => {
  const nameChunks = [structure.name];
  if (
    // if the trailing slash option is enabled
    options.trailingDirSlash && // and if the item has at least one child
    structure.children.length > 0 && // and if the item doesn't already have a trailing slash
  ...
  if (options.fullPath && structure.parent && structure.parent) {
    nameChunks.unshift(
      getName(
        structure.parent,
        (0, import_lodash.defaultsDeep)({}, { trailingDirSlash: true }, options)
      )
    );
  }
  return nameChunks.join("");
};
var isLastChild = (structure) => Boolean(structure.parent && (0, import_lodash.last)(structure.parent.children) === structure);

// api/utilities/parse-input.ts
var import_lodash2 = __toESM(require_lodash());
var leadingWhitespaceAndBulletRegex = /^((\s*)(?:-\s)?)/;
var onlyWhitespaceRegex = /^\s*$/;
var newlineSplitterRegex = /[^\r\n]+/g;
var parseInput = (input) => {
  ...
  for (const s of structures) {
    while ((0, import_lodash2.last)(path).indentCount >= s.indentCount) {
      path.pop();
    }
    const parent = (0, import_lodash2.last)(path);
...
// api/markdown-tree-api.ts
var MarkdownTreeApi = class {
  constructor() {
  }
  handle(settings, source, element, context) {
    const options = {
...
// api/consts/default-settings.ts
...
// api/commands/new-tree-block-command.ts
var NewTreeBlockCommand = class {
  constructor() {
    this.id = "new-tree-block-command";
    this.name = "New tree block";
  }
...
// main.ts
var MarkdownTree = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
    this.api = new MarkdownTreeApi();
  }
  async onload() {
...
var ObsidianTreeSettingsTabSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Fancy").setDesc("Enables/disables the fancy characters of the tree").addToggle((text) => text.setValue(this.plugin.settings.fancy).onChange(async (value) => {
      this.plugin.settings.fancy = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Full path").setDesc("Enables/disables whether it shows the full path or not").addToggle((text) => text.setValue(this.plugin.settings.fullPath).onChange(async (value) => {
      this.plugin.settings.fullPath = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Trailing /").setDesc('Enables/disables whether it shows the trailing "/" or not').addToggle((text) => text.setValue(this.plugin.settings.trailing).onChange(async (value) => {
      this.plugin.settings.trailing = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Root .").setDesc('Enables/disables whether it shows the Root "." in the beginning of the tree or not').addToggle((text) => text.setValue(this.plugin.settings.root).onChange(async (value) => {
